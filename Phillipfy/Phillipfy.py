...
class Phillipfy():
    ...
    def __init__(self,room=None):
        '''
        This method is called when an instance of the Phillipfy class is created
            :param - room - select the room you want to run the app in. Default is None, which will subsequently default to the first group.
        '''
        ...
        self._lights = [int(x) for x in self._bridge.get_group(self._group)['lights']]
        ...
        colors = self._util.extract_colors(self._current_song['item']['album']['images'][0]['url'])
        self._set_colors(colors)
    
    def _get_group_from_room(self, room):
        '''
        Search thru the available groups and look for one with a name that matches the one the app was init with
        '''
    ...
    def _set_colors(self,colors):
        ...
        Set the colors for the lights
            :param - colors - a list (or tuple) of rgb tuples. Must be the same length as the number of lights in the room
        '''
        if len(self._lights) != len(colors):
            print('Error! Color list must be same length as number of lights in the room!')

        print('Changing colors ... ')
        for light_id, color in zip(self._lights,colors):
            self._bridge.set_light(light_id,'xy',self._util.rgb_to_xy(color.rgb.r, color.rgb.g, color.rgb.b))
